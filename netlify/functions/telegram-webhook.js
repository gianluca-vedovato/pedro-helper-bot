...
import { getSupabase, pollsGet, pollsUpdateResults, pollsUpsert, ruleExists, rulesDelete, rulesGetAll, rulesNextNumber, rulesUpsert } from '../../src/services/db';
    ...
    if (!bot) {
        ...
        bot.command('regolamento', async (ctx) => {
            ...
            if (arg) {
                ...
                if (!found)
                    return ctx.reply(`❌ Regola ${n} non trovata.`);
        ...
        bot.command('askpedro', async (ctx) => {
            ...
            if (!rules.length)
                return ctx.reply('❌ Nessuna regola caricata.');
            const rulesText = rules.map((r) => `${r.rule_number}. ${r.content}`).join('\n\n');
        ...
        bot.command('promemoria_cancella', async (ctx) => {
            ...
            if (error)
                return ctx.reply('❌ Errore nella cancellazione.');
            if (!data?.length)
                return ctx.reply('❌ Promemoria non trovato o non autorizzato a cancellarlo.');
        ...
        bot.on('poll', async (ctx) => {
            ...
            const results = Object.fromEntries((poll.options || []).map((o) => [o.text, o.voter_count]));
        ...
        bot.action(/apply:.+/, async (ctx) => {
            ...
            if (!poll_id)
                return ctx.answerCbQuery('ID sondaggio mancante');
            const chat_id = ctx.chat?.id;
        ...
        bot.command('sondaggio_manuale', async (ctx) => {
            ...
            if (parts.length < 2)
                return ctx.reply('Uso: /sondaggio_manuale "Domanda" "Opzione vincente" ["Opz1|Opz2|..."]');
            const chat_id = ctx.chat?.id;
            const user_id = ctx.from?.id;
            const isAdmin = await isUserAdmin(chat_id, user_id);
            if (!isAdmin)
                return ctx.reply('❌ Solo gli amministratori possono applicare un sondaggio manuale.');
            const question = parts[0];
            const winning = parts[1];
            const options = parts[2] ? parts[2].split('|').map((s) => s.trim()).filter(Boolean) : undefined;
...
async function handleApplyPoll(ctx, poll_id) {
    ...
    if (!pollRow) {
        ...
        return ctx.reply('❌ Sondaggio non trovato nel database. Rispondi al messaggio del sondaggio con /applica_sondaggio oppure riprova più tardi.');
    }
...
async function applyDecisionFromAI(ctx, params) {
    console.log('🤖 applyDecisionFromAI chiamata con parametri:', params);
    const rules = params.rulesText || (await rulesGetAll()).map((r) => `${r.rule_number}. ${r.content}`).join('\n\n');
    ...
    try {
        ...
        if (!action) {
            ...
            return ctx.reply('❌ Non sono riuscito a capire l\'azione dal sondaggio. Riformula la domanda o rendi più chiare le opzioni (es. sì/no).');
        }
        if (action === 'remove') {
            if (rule_number == null)
            ...
            if (!existed)
                return ctx.reply(`ℹ️ La regola ${rule_number} non esiste già. Nessuna rimozione effettuata.`);
            await rulesDelete(rule_number);
            return ctx.reply(`✅ Regola ${rule_number} rimossa con successo.`);
        }
        if (rule_number == null && action === 'add')
            rule_number = await rulesNextNumber();
        if (!proposed_content)
            return ctx.reply('❌ Nessun contenuto proposto trovato. Rendi più chiara la domanda/risposta.');
        console.log('💾 Salvo regola:', { rule_number, proposed_content });
        const ok = await rulesUpsert(rule_number, proposed_content);
        if (!ok)
